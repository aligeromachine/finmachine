...
import { PROFIT_URL, PROFIT_ADD, PROFIT_EDIT, PROFIT_DEL, PROFIT_ROW } from './const';
...
export const addProfitRow = async () => {
    ...
    const response = await postRequestCheck(PROFIT_URL, params);
...
export const deleteProfitRow = async pk => {
    ...
    await postRequestCheck(PROFIT_URL, params);
    await store.dispatch(getProfitTable());
};
