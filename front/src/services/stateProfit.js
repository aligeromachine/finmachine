import { createSlice, createAsyncThunk } from "@reduxjs/toolkit";
import { apiClient } from "../utils/requests";

const initialState = {
  recordsTotal: 0,
  offset: 0,
  recordsDisplay: 0,
  draw: [],
  loading: "loading" | "idle" | "failed",
};

export const getProfitThunk = createAsyncThunk(
  "stateProfit/getProfitThunk",
  async (data) => {
    const response = await apiClient.post("/profit/table/", data);
    return response;
  },
);

export const stateProfit = createSlice({
  name: "stateProfit",
  initialState: initialState,
  reducers: {},
  extraReducers: (builder) => {
    builder
      .addCase(getProfitThunk.pending, (state) => {
        state.loading = "loading";
      })
      .addCase(getProfitThunk.fulfilled, (state, action) => {
        state.recordsTotal = action.payload.recordsTotal;
        state.offset = action.payload.offset;
        state.recordsDisplay = action.payload.recordsDisplay;
        state.draw = action.payload.draw;
        state.loading = "idle";
      })
      .addCase(getProfitThunk.rejected, (state) => {
        state.loading = "failed";
      });
  },
});

export const profitReducer = stateProfit.reducer;
